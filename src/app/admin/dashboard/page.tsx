...
import { Button } from "@/components/ui/button";
import { Users, Calendar, DollarSign, TrendingUp, ArrowRight, UserCheck, RefreshCw, Loader2 } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Chart as ChartJS, CategoryScale, LinearScale, BarElement, PointElement, LineElement, Title, Tooltip, Legend } from "chart.js";
import { Line, Bar } from "react-chartjs-2";
import { useAdminDashboard } from "@/hooks/useAdminDashboard";
import { useAdminNotifications } from "../hook/useAdminNotifications";
import { formatCurrency, formatPercentage, formatTimeAgo } from "@/lib/utils/formatters";

ChartJS.register(CategoryScale, LinearScale, BarElement, PointElement, LineElement, Title, Tooltip, Legend);

export default function DashboardPage() {
  const { dashboardData, loading, error, refreshData } = useAdminDashboard();
  ...
  const { stats, charts, recentUsers } = dashboardData;
  ...
  const statsConfig = [
    { title: "Total Users", value: safeStats.totalUsers.toString(), change: formatPercentage(safeStats.totalUsersChange), icon: Users, color: "text-blue-600", bgColor: "bg-blue-100", trend: safeStats.totalUsersChange >= 0 ? "up" : "down" },
    { title: "Active Bookings", value: safeStats.activeBookings.toString(), change: formatPercentage(safeStats.activeBookingsChange), icon: Calendar, color: "text-purple-600", bgColor: "bg-purple-100", trend: safeStats.activeBookingsChange >= 0 ? "up" : "down" },
    { title: "Monthly Revenue", value: formatCurrency(safeStats.monthlyRevenue), change: formatPercentage(safeStats.monthlyRevenueChange), icon: DollarSign, color: "text-green-600", bgColor: "bg-green-100", trend: safeStats.monthlyRevenueChange >= 0 ? "up" : "down" },
    { title: "Growth Rate", value: formatPercentage(safeStats.growthRate), change: formatPercentage(safeStats.growthRateChange), icon: TrendingUp, color: "text-orange-600", bgColor: "bg-orange-100", trend: safeStats.growthRateChange >= 0 ? "up" : "down" },
  ];
  ...
  const revenueData = {
    labels: charts?.weeklyRevenue?.map((item) => item.day) ?? [],
    datasets: [{ label: "Revenue", data: charts?.weeklyRevenue?.map((item) => item.value) ?? [], borderColor: "#3b82f6", backgroundColor: "rgba(59,130,246,0.2)", borderWidth: 2, tension: 0.4 }]
  };

  const appointmentsData = {
    labels: charts?.weeklyAppointments?.map((item) => item.day) ?? [],
    datasets: [{ label: "Appointments", data: charts?.weeklyAppointments?.map((item) => item.value) ?? [], backgroundColor: "#8b5cf6", borderRadius: 6 }]
  };

  const handleNavigate = (tab: string) => console.log("Navigate to:", tab);
            ...
            recentUsers.map((user, index) => (
              <div key={user.email ?? index} className="flex justify-between items-center">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-gray-500 mt-1">{formatTimeAgo(user.registeredAt)}</p>
                </div>
              </div>
            ))
          )}
        ...
        <CardContent className="space-y-4">
          {notifications.length === 0 ? (
            ...
            notifications.map((alert, index) => (
              <div key={alert.timestamp + index} className="p-3 rounded-lg border border-l-4 border-blue-600 bg-blue-50">
                <div className="flex justify-between">
                  <span className="text-sm">{alert.message}</span>
                  <span className="text-xs text-gray-500">{alert.timestamp}</span>
                </div>
                <Button variant="ghost" size="xs" className="mt-2" onClick={() => markAsRead(index)}>Mark as read</Button>
              </div>
            ))
          )}
        </CardContent>
