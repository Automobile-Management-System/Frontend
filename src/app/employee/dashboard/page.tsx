...
export default function EmployeeDashboard() {
  ...
  const getServiceField = (
    ...
    switch (fieldName) {
      ...
      case "serviceName":
        value =
          service.serviceName || service.ServiceName || service.service_name;
        break;
      case "customerName":
        value =
          service.customerName || service.CustomerName || service.customer_name;
        break;
      case "address":
  ...
  const getModificationStatus = (modification: any) => {
    ...
    const status =
      modification.appointmentStatus ||
      modification.status ||
      modification.Status ||
  ...
  const getModificationField = (
    ...
    switch (fieldName) {
      ...
      case "customerName":
        value =
          modification.customerName ||
          `Vehicle ${modification.vehicleId}` ||
          "Unknown Customer";
        break;
      case "dueDate":
  ...
  const formatStatus = (status: string | undefined | null) => {
    ...
    // Change "Pending" to "Upcoming" in the UI
    return formatted === "Pending" ? "Upcoming" : formatted;
  };
    ...
    <div className="space-y-6 p-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-gray-900">
                Today Recent Services
              </h2>
            </div>
            <p className="text-sm text-gray-500 mt-1">
              Your recent services for{" "}
              {recentServices?.date ? formatDate(recentServices.date) : "today"}
            </p>
          </div>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          ...
                          • {getServiceField(service, "address", "No address")}
                        </p>
                      </div>
                    </div>
                    <span
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Your recent modifications for{" "}
              {recentModifications?.date
                ? formatDate(recentModifications.date)
                : "today"}
            </p>
          </div>
          ...
          <div className="p-6 space-y-4">
            {recentModifications?.recentModifications?.length ? (
              recentModifications.recentModifications.map(
                (modification, index) => {
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div className="text-center">
                          <p className="text-sm font-medium text-gray-900">
                            {formatTime(
                              getModificationField(modification, "date", null)
                            )}
                          </p>
                        </div>
                        <div>
                          <h3 className="text-sm font-medium text-gray-900">
                            {getModificationField(
                              modification,
                              "projectName",
                              "Unknown Project"
                            )}
                      ...
                      <span
                        className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(
                          getModificationStatus(modification)
                        )}`}
                      >
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <button
            onClick={() =>
              (window.location.href = "/employee/service_progress")
            }
            className="flex items-center justify-center p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition"
          >
